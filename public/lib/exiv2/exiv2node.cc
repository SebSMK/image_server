...
static Handle<Value> SetImageTags(const Arguments& args) {
  ...
  for (unsigned i = 0; i < keys->Length(); i++) {
    ...
    /* The following error message appears if you attempt to write non-ascii characters (©, æ,å ,ø etc.)
     * to XMP : "Error: XMP toolkit error 4: Invalid UTF-8 sequence length"
     *
     * This issue seems to describe the problem and solution when using exiv2 from a terminal window:
     * http://dev.exiv2.org/boards/3/topics/1073 "The XMP value must be in UTF-8 but Exiv2 doesn't translate 
     * the input (a missing feature....). If your Terminal uses UTF-8, you won't have that problem, else it 
     * is an issue".
     */
